var createEditor = (function(editorid, filename, url) {
					...
					document.getElementById(editorid),
					{
						...
						onKeyEvent : function(i, e) {
							if (!done) {
								...
								complete_comment.parentNode
										.removeChild(complete_comment);
							}
							// Hook into ctrl-s
		...
		connect(sel, "keydown", function(event) {
			...
			// Escape, backspace, left, right
				...
				editor.focus();
					...
					&& sel.selectedIndex == completions.length - 1) {
				close();
				editor.focus();
	...
	function getCompletions(token) {
		...
		if (start.length == 0)
			...
			var kw = rb_keywords.concat(rb_builtinfunction,
					rb_functionfornumbers, rb_functionforfloat,
					rb_functionformath).sort();
			kw = allVar.concat(kw);
